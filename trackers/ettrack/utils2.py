...
class Trajectory_Dataloader():
    def __init__(self, args):
        ...
        self.TRAIN_DATA_PATH = args.train_data_path   #'datasets/MOT17/train'
        self.TEST_DATA_PATH = args.val_data_path   #'datasets/MOT17/val'

        if True:
            self.data_dirs = os.listdir(self.TRAIN_DATA_PATH)
            test_dirs = os.listdir(self.TEST_DATA_PATH)
            for seq in sorted(self.data_dirs):
                if '.DS_Store' in seq or '.ipy' in seq or '.txt' in seq:  # pmb added .txt
        ...
        self.train_data_file = Path(self.args.save_dir)/Path( f"{args.dataset}_train_trajectories.cpkl")
        self.test_data_file = Path(self.args.save_dir)/Path(f"{args.dataset}_test_trajectories.cpkl")
        self.train_batch_cache = Path(self.args.save_dir) /Path(f"{args.dataset}_train_batch_cache.cpkl")
        self.test_batch_cache = Path(self.args.save_dir) /Path(f"{args.dataset}_test_batch_cache.cpkl")

        logger.info("Load ground truth files.")
        if not self.train_data_file.is_file() or self.args.refresh_cache:
            self.traject_preprocess('train')
        else:
            logger.info(f'Loaded {self.train_data_file} from cache.')
        if not self.test_data_file.is_file() or self.args.refresh_cache:
            self.traject_preprocess('test')
        else:
            logger.info(f'Loaded {self.test_data_file} from cache.')
        logger.info("Done.")

        # Load the processed data from the pickle file
        logger.info("Preparing data batches.")
        if not self.train_batch_cache.is_file() or self.args.refresh_cache:

            logger.debug(f"Creating pre-processed training data batches in {self.train_batch_cache}")
        ...
        if not self.test_batch_cache.is_file() or self.args.refresh_cache:

            logger.debug(f"Creating pre-processed test  data batches in {self.test_batch_cache}")
        ...
        logger.info(f'Total number of training batches: {self.trainbatchnums}')
        logger.info(f'Total number of test batches: {self.testbatchnums}')

        self.reset_batch_pointer(set='train', valid=False)
        self.reset_batch_pointer(set='train', valid=True)   # validation not valid
