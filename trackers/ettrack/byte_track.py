...
import numpy as np
from .byte.STrack import STrack
import torch
import trackers.ettrack.byte.matching as matching
from .byte.basetrack import TrackState
...
class byte_tp(object):
        ...
        for i in range(track_num):
            ...
            if array_representation.shape[0] == fram_tp:
                ...
                if array_representation.shape[0] == 1:
                    ...
                    tp_last = np.concatenate(
                        (np.expand_dims(array_representation[-1], axis=0), np.expand_dims(offset_last[-1], axis=0)),
                        axis=1)
    ...
    def update(self, output_results, img_info, img_size):
        """
        update the tracker
        :param output_results: 2d array (num peds x (bbox, score, ? ,?)
        :param img_info:
        :param img_size:
        :return:
        """
