...
class MOTEvaluator:
    ...
    def evaluate_ocsort(
        ...
        for cur_iter, (imgs, _, info_imgs, ids) in enumerate(
            ...
            with torch.no_grad():
                ...
                if os.path.exists(ckt_file) and self._use_saved_detections:
                    ...
                    # PMB added this back in as a flag
                    if self._use_saved_detections:
    ...
    def evaluate_hybrid_sort(
        ...
        """
        # TODO half to amp_test
        logger.debug("Evaluate hybrid sort")
        tensor_type = torch.cuda.HalfTensor if half else torch.cuda.FloatTensor
        model = model.eval()
        if half:
            model = model.half()
        ...
        for cur_iter, (imgs, _, info_imgs, ids, raw_image) in enumerate(    # [hgx0411] add raw_image for FastReID
            ...
            with torch.no_grad():
                ...
                ckt_file = Path("dance_detections/dancetrack_eval_hybrid_sort/{}/{}_detetcion.pkl".format(video_name, img_base_name))
                if ckt_file.is_file() and self._use_saved_detections:
                    data = torch.load(ckt_file)
                    outputs = [data['detection']]
                else:
                    imgs = imgs.type(tensor_type)
                    ...
                    if self._use_saved_detections:
                        ...
                        ckt_file.parent.mkdir(parents=True, exist_ok=True)
                        #os.makedirs("dance_detections/{}".format(video_name), exist_ok=True)
